...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabaseServer.from('sos_alerts').select('*').order('created_at', { ascending: false });
...
export async function POST(req: Request) {
  ...
  if (action === 'responded') {
    const { error } = await supabaseServer.from('sos_alerts').update({ status: 'RESOLVED' }).eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
